...
// 5a) Crear un array que contenga 5 palabras y recorrer dicho array utilizando
// un bucle for de JavaScript para mostrar una alerta utilizando cada una de las palabras.
var fiveWordsArray = ["one", "two", "three", "four", "five"];
for (let i = 0; i < fiveWordsArray.length; i++) {
    const element = fiveWordsArray[i];
    alert(element);
}
...
for (let i = 0; i < fiveWordsArray.length; i++) {
    const element = fiveWordsArray[i];
    const elementModified = element.substring(0, 1).toLocaleUpperCase() + element.substring(1);
    alert(elementModified);
...
// 5d) Crear una array vacío y con un bucle for de 10 repeticiones. Llenar el array con el número 
